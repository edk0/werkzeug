...
class MultiPartParser(object):
    ...
    class LineSplitter(object):
        ...
        def feed(self, data):
            ...
            lines, self.buffer = self._splitlines(tmp)
            if not data:
                lines += [self.buffer]
                if self.buffer:
                    lines += [b'']
        ...
        def _start_content(self):
            ...
            if transfer_encoding is not None:
                ...
                try:
                    ...
                    raise ValueError('Cannot decode transfer-encoding: %r' % transfer_encoding)
            self.name = extra.get('name')
            self.filename = extra.get('filename')
            if self.filename is not None:
                self._output.append(('begin_file', (self._headers, self.name, self.filename)))
            else:
        ...
        def _feed_one(self, event):
            ...
            if ev == 'begin_file':
                ...
                if self._is_file:
                    ...
                    return ('file',
                            (self._name, FileStorage(self._container,
                                                     self._filename, self._name,
                                                     headers=self._headers)))
                else:
                    part_charset = p.get_part_charset(self._headers)
                    return ('form',
                            (self._name, b''.join(self._container).decode(
                                part_charset, p.errors)))
